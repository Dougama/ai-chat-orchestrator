...
export class TokenTrackingService {
  ...
  private static calculateCost(usage: TokenUsage): { 
    ...
    const pricing = this.PRICING[model as keyof typeof this.PRICING] || this.PRICING['gemini-2.0-flash-001'];
    ...
    await firestore.runTransaction(async (transaction) => {
      ...
      if (chatDoc.exists) {
        ...
        transaction.update(chatUsageRef, {
          ...
          'costBreakdown.generationCost': FieldValue.increment(generationCostIncrement),
          'costBreakdown.embeddingCost': FieldValue.increment(embeddingCostIncrement),
          'tokenBreakdown.generationTokens': FieldValue.increment(generationTokensIncrement),
          'tokenBreakdown.embeddingTokens': FieldValue.increment(embeddingTokensIncrement)
        });
      } else {
  ...
  private static async updateGlobalUsage(
    ...
    await firestore.runTransaction(async (transaction) => {
      ...
      if (globalDoc.exists) {
        ...
        monthlyBreakdown[currentMonth].generationCost += generationCostIncrement;
        monthlyBreakdown[currentMonth].embeddingCost += embeddingCostIncrement;
        monthlyBreakdown[currentMonth].generationTokens += generationTokensIncrement;
        monthlyBreakdown[currentMonth].embeddingTokens += embeddingTokensIncrement;
   ...
   */
  static async getChatUsage(firestore: Firestore, chatId: string): Promise<ChatUsageData | null> {
    try {
      const chatUsageRef = firestore.collection('AI').doc(chatId);
   ...
   */
  static async getGlobalUsage(firestore: Firestore): Promise<GlobalUsageData | null> {
    try {
      const globalUsageRef = firestore.collection('AI').doc('global');
   ...
   */
  static async getUsageStats(firestore: Firestore, startDate?: Date, endDate?: Date): Promise<{
    totalChats: number;
    totalTokens: number;
    totalCost: number;
  ...
  private static processUsageStats(snapshot: any): {
    ...
    try {
      ...
      console.error('TokenTrackingService: Error processing usage stats:', error);
      return {
        totalChats: 0,
        totalTokens: 0,
        totalCost: 0,
